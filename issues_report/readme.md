...
- ปัญหา SET ไม่สามารถส่งเมลไปยัง Google Mail
    - Service Provider ส่วนใหญ่บล็อก SMTP สำหรับการส่งเมลที่มีลักษณะ `phishing` หรือมี `payload` อันตราย
    - ทำให้ฝั่งผู้รับที่ใช้ Google Mail จะไม่ได้รับเมลใดๆ ที่ส่งมาจาก SET

- แก้ไขโดยใช้ MailHog แทน Google Mail ในฝั่งผู้รับ
    - สามารถทำงานเป็น SMTP server ในการรับไฟล์ได้
...
- ปัญหาการแก้ไข `Source Code` ของ `JohnHammond` ปัจจุบันยังไม่สามารถควบรวมเข้ากับ `Command` โหลด `Payload` ได้
   - ยังไม่สามารถเข้าใจโครงสร้างการทำงานของ code ได้อย่างถ่องแท้
   - จึงคาดว่าจะไม่สามารถแก้ไขปัญหานี้ได้ทันก่อนวันนำเสนอ
 
- ส่วนนี้เป็นปัญหาที่ใช้เวลานานที่สุดตั้งแต่เริ่มจนจบ อาจเป็นเพราะยังให้เวลากับ Project นี้ได้ไม่มากพอ

## 06/09/2025
- ปัญหา Session ของ Metasploit มันทับซ้อนกัน
    - ทำให้เข้าถึงเครื่องเหยื่อได้ แต่คำสั่งจะไม่สามารถไปเครื่องของเหยื่อได้
 
- เมื่อ Reconnect กลับไปใหม่ ทำให้ Session กลับมาปกติ แต่ว่าไม่ทันแล้วเพราะว่าไม่สามารถแก้ไขปัญหาหน้างานได้ทัน
